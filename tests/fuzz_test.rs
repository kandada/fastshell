...
fn setup() -> Fastshell {
    ...
    sdk.write_file("words.txt", "apple\nBanana\nAPPLE\ncherry\ndate\n")
        .unwrap();
    sdk.write_file("json.txt", r#"{"key": "value", "nested": {"x": 1}}"#)
        .unwrap();
...
fn fuzz_path_escape_variants() {
    ...
    for path in escape_attempts {
        ...
        assert!(
            r.is_err(),
            "Path '{}' should be blocked but got: {:?}",
            path,
            r
        );
...
"#;
    ...
    eprintln!(
        "os.popen result: stdout={:?} stderr={:?}",
        r.stdout, r.stderr
    );
    // os.popen should either be hooked or fail gracefully
    assert!(
        r.exit_code == 0 || r.stderr.contains("ERROR") || r.stderr.contains("AttributeError"),
        "os.popen should be handled: code={} stdout={:?} stderr={:?}",
        r.exit_code,
        r.stdout,
        r.stderr
    );
...
"#;
    ...
    assert!(
        r.exit_code == 0,
        "ctypes should at least not panic: code={} stdout={:?} stderr={:?}",
        r.exit_code,
        r.stdout,
        r.stderr
    );
...
"#;
    ...
    assert!(
        r.stdout.contains("shlex_test") || r.stderr.contains("ERROR"),
        "subprocess.run with list args: stdout={:?} stderr={:?}",
        r.stdout,
        r.stderr
    );
}
...
"#;
    ...
    assert!(
        ...
        "VFS path escape should be blocked in Python: {}",
        r.stdout
    );
}
