...
impl Shell {
    pub fn cmd_bzip2(&self, args: &[&str]) -> CommandOutput {
        ...
        if files.is_empty() {
            return CommandOutput::error("bzip2: missing file operand\n".to_string(), 1);
        }

        let mut output_bytes = Vec::new();
        for file in &files {
            let input = match self.vfs.read(file, &self.cwd) {
                Ok(b) => b,
                Err(e) => return CommandOutput::error(format!("bzip2: {}: {}\n", file, e), 1),
            };
            ...
            if to_stdout {
                ...
                if let Err(e) = self.vfs.write_bytes(&out, &self.cwd, &compressed) {
                    return CommandOutput::error(format!("bzip2: {}: {}\n", out, e), 1);
                }
                if !keep {
    ...
    pub fn cmd_bunzip2(&self, args: &[&str]) -> CommandOutput {
        ...
        for file in &files {
            let compressed = match self.vfs.read(file, &self.cwd) {
                Ok(b) => b,
                Err(e) => return CommandOutput::error(format!("bunzip2: {}: {}\n", file, e), 1),
            };
            ...
            if to_stdout {
                ...
                if let Err(e) = self.vfs.write_bytes(out, &self.cwd, &decompressed) {
                    return CommandOutput::error(format!("bunzip2: {}: {}\n", out, e), 1);
                }
                if !keep {
                    let _ = self.vfs.remove_file(file, &self.cwd);
    ...
    pub fn cmd_xz(&self, args: &[&str]) -> CommandOutput {
        ...
        if files.is_empty() {
            return CommandOutput::error("xz: missing file operand\n".to_string(), 1);
        }
        let mut output_bytes = Vec::new();
        for file in &files {
            let input = match self.vfs.read(file, &self.cwd) {
                Ok(b) => b,
                Err(e) => return CommandOutput::error(format!("xz: {}: {}\n", file, e), 1),
            };
            let mut compressed = Vec::new();
            {
            ...
            if to_stdout {
                ...
                let out = format!("{}.xz", file);
                self.vfs.write_bytes(&out, &self.cwd, &compressed).ok();
                if !keep {
                    let _ = self.vfs.remove_file(file, &self.cwd);
                }
            }
    ...
    pub fn cmd_unxz(&self, args: &[&str]) -> CommandOutput {
        ...
        for file in &files {
            let compressed = match self.vfs.read(file, &self.cwd) {
                Ok(b) => b,
                Err(e) => return CommandOutput::error(format!("unxz: {}: {}\n", file, e), 1),
            };
            let mut decompressed = Vec::new();
            if let Err(e) =
            ...
            if to_stdout {
                ...
                let out = file.strip_suffix(".xz").unwrap_or(file);
                self.vfs.write_bytes(out, &self.cwd, &decompressed).ok();
                if !keep {
                    let _ = self.vfs.remove_file(file, &self.cwd);
                }
            }
    ...
    pub fn cmd_zcat(&self, args: &[&str]) -> CommandOutput {
        ...
        if files.is_empty() {
            return CommandOutput::error("zcat: missing file operand\n".to_string(), 1);
        }
        let mut output = Vec::new();
        for file in &files {
            let data = match self.vfs.read(file, &self.cwd) {
                Ok(b) => b,
                Err(e) => return CommandOutput::error(format!("zcat: {}: {}\n", file, e), 1),
            };
            let mut d = Vec::new();
    ...
    pub fn cmd_dos2unix(&self, args: &[&str]) -> CommandOutput {
        ...
        for file in &files {
            ...
            if let Err(e) = self.vfs.write(file, &self.cwd, &converted) {
                return CommandOutput::error(format!("dos2unix: {}: {}\n", file, e), 1);
            }
        }
        CommandOutput::success(String::new())
    }
    ...
    pub fn cmd_unix2dos(&self, args: &[&str]) -> CommandOutput {
        ...
        for file in &files {
            ...
            let converted = content.replace("\n", "\r\n");
            if let Err(e) = self.vfs.write(file, &self.cwd, &converted) {
                return CommandOutput::error(format!("unix2dos: {}: {}\n", file, e), 1);
            }
        }
        CommandOutput::success(String::new())
    }
