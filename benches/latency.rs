...
fn setup() -> Fastshell {
    ...
    .unwrap();
    for i in 0..100 {
        sdk.write_file(&format!("file_{}.txt", i), &format!("content {}", i))
            .unwrap();
    }
...
fn bench_ls(c: &mut Criterion) {
    ...
    group.bench_function("ls", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("ls")));
        })
    });

    group.bench_function("ls_la", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("ls -la")));
        })
    });

    group.bench_function("pwd", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("pwd")));
        })
    });

    group.bench_function("echo", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("echo hello")));
        })
    });

    group.bench_function("cat_small", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("cat file_0.txt")));
        })
    });

    group.bench_function("mkdir", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("mkdir bench_dir")));
        })
    });

    group.bench_function("grep_simple", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("grep content file_0.txt")));
        })
    });

    group.bench_function("wc", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("wc -l file_0.txt")));
        })
    });
...
fn bench_pipeline_latency(c: &mut Criterion) {
    ...
    group.bench_function("echo_grep_wc", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("echo hello world | grep hello | wc -w")));
        })
    });

    group.bench_function("ls_grep", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("ls | grep file")));
        })
    });

    group.bench_function("cat_wc", |b| {
        b.iter(|| {
            let _ = black_box(sdk.execute(black_box("cat file_0.txt | wc -c")));
        })
    });
...
fn bench_filesystem_latency(c: &mut Criterion) {
    ...
    group.bench_function("read_file", |b| {
        let sdk = setup();
        b.iter(|| {
            let _ = black_box(sdk.read_file(black_box("file_0.txt")));
        })
    });
    ...
    group.bench_function("write_file", |b| {
        let sdk = setup();
        b.iter(|| {
            let _ = black_box(sdk.write_file(black_box("bench_write.txt"), black_box("data")));
        })
    });
    ...
    group.bench_function("exists_check", |b| {
        let sdk = setup();
        b.iter(|| {
            let _ = black_box(sdk.exists(black_box("file_0.txt")));
        })
    });
    ...
    group.bench_function("list_dir", |b| {
        let sdk = setup();
        b.iter(|| {
            let _ = black_box(sdk.list_dir(black_box("/")));
        })
    });
